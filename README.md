...
# xia Yue Pro V3.1 版本更新日志

> 致敬 AI 时代的古法渗透：工具可以更聪明，但验证请求、对比响应、确认授权边界，依然是安全测试的基本功。

## 主要更新

- 支持替换请求行、URL、表单以及 POST 请求体中的认证参数。
- 支持 `application/json` 请求体中的认证参数替换，包括嵌套 JSON 和同名字段精确匹配。
- 新增参数变异测试：空值、`0`、`-1`，用于泛查询和边界 ID 场景。
- 新增已记录请求回放：更新 Cookie、Authorization 等认证头后，可重新回放全部记录请求。
- 新增请求路径过滤，可按精确路径或 `*` 前缀匹配跳过受保护路径。
- 新增结果表格排序和参数变异结果展示。
- 新增可选日志开关，默认关闭；关闭日志时仍输出基本启动信息，便于确认插件是否成功加载。
- 优化配置页分区、请求/响应查看布局以及多条变异结果的显示。

## 下载

- JDK 8 和 JDK 11 编译产物请前往 [3.1 Release](https://github.com/h00klod0er/xia_Yue_pro/releases/tag/3.1) 下载。
